...
/** Shallow compare for optional prop bags (e.g. FlatList rest props) to keep stable references. */
...
export const calculateMaxOffset = (
  ...
  return {
    x:
      scaledContentWidth < containerSize.width
        ? 0
        : (scaledContentWidth - containerSize.width) / 2 / scale,
    y:
      scaledContentHeight < containerSize.height
        ? 0
